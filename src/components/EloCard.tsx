import { getElo, getNextElo } from "@/lib/elo";
...
export default function EloCard({ viewCount, referralCount }: EloCardProps) {
  const elo = getElo(viewCount, referralCount);
  const next = getNextElo(viewCount, referralCount);

  const isX = elo.tier === "x";
  const isDark = ["prata", "platina", "diamante"].includes(elo.tier);
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: `0 0 16px ${elo.glowColor}`,
      }}
    >
          ...
          <div className="flex items-center gap-2">
            <span
              ...
              style={{
                ...
                boxShadow: `0 2px 8px ${elo.glowColor}`,
              }}
            >
              {elo.label}
            </span>
            <span className="text-xs text-muted-foreground font-medium">Ranking</span>
          </div>
          {isX && (
            <span className="text-xs font-bold" style={{ color: elo.color }}>
              ✦ Nível máximo
            </span>
          )}
        </div>
