export type NivelTier = "ferro" | "bronze" | "prata" | "ouro" | "platina" | "diamante" | "esmeralda" | "x";
...
const TIERS: { tier: NivelTier; minViews: number; minReferrals: number }[] = [
...
const NIVEL_META: Record<NivelTier, Omit<NivelInfo, "tier">> = {
  ferro: {
    label: "Ferro",
    color: "#8a8a8a",
...
export function getNivel(viewCount: number, referralCount: number): NivelInfo {
  for (const t of TIERS) {
    if (viewCount >= t.minViews && referralCount >= t.minReferrals) {
      return { tier: t.tier, ...NIVEL_META[t.tier] };
    }
  }
  return { tier: "ferro", ...NIVEL_META.ferro };
}

export function getNextNivel(viewCount: number, referralCount: number): { tier: NivelTier; label: string; missingViews: number; missingReferrals: number } | null {
  const current = getNivel(viewCount, referralCount);
  const idx = TIERS.findIndex(t => t.tier === current.tier);
  if (idx === 0) return null; // já é X
  const next = TIERS[idx - 1];
  return {
    tier: next.tier,
    label: NIVEL_META[next.tier].label,
